...
$bairro = [];
try {
    $stmt = $conn->prepare("SELECT * FROM faeterj_apoio_bairro");
    $stmt->execute();
    $bairro = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "Erro ao buscar bairro: " . $e->getMessage();
}
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <div class="container mt-5 mb-5">
    ...
    <div class="bg-primary text-white rounded px-3 py-2 mb-4">
      <h4 class="mb-0">Lista de Bairros Cadastrados</h4>
    </div>
    ...
    <?php if (!empty($bairro)): ?>
      <div class="table-responsive border border-azul rounded bg-white shadow-sm p-3">
        <table class="table table-bordered align-middle table-striped">
          ...
          <tbody>
            <?php foreach ($bairro as $row): ?>
              <tr>
                <td class="text-center"><?= htmlspecialchars($row['bairro_desc']) ?></td>
                <td class="text-center">
                  <a href="../updates/updates_bairros.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Editar</a>
                  <a href="../deletes/deletes_bairro.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Deseja excluir este registro?')">Excluir</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
