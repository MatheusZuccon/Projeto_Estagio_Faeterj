...
$seguradora = [];
try {
    $stmt = $conn->prepare("SELECT * FROM faeterj_apoio_seguradora");
    $stmt->execute();
    $seguradora = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "Erro ao buscar seguradoras: " . $e->getMessage();
}
...
<head>
  <meta charset="UTF-8">
  <title>Seguradoras Cadastradas</title>
...
<!-- Bootstrap JS (adicione antes do fechamento do body) -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>

  <div class="container mt-5 mb-5">
    <!-- Cabeçalho Azul -->
    <div class="bg-primary text-white rounded px-3 py-2 mb-4">
      <h4 class="mb-0">Lista de Seguradoras Cadastradas</h4>
    </div>

    <?php if (!empty($seguradora)): ?>
      <div class="table-responsive border border-azul rounded bg-white shadow-sm p-3">
        <table class="table table-bordered align-middle table-striped">
          ...
          <tbody>
            <?php foreach ($seguradora as $row): ?>
              <tr>
                <td class="text-center"><?= htmlspecialchars($row['nome']) ?></td>
                <td class="text-center">
                  <a href="../updates/updates_seguradora.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Editar</a>
                  <a href="../deletes/deletes_seguradora.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Deseja excluir este registro?')">Excluir</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
