...
$cursos = [];
try {
    $stmt = $conn->prepare("SELECT * FROM faeterj_apoio_curso");
    $stmt->execute();
...
<div class="container mt-5 mb-5">
   <div class="bg-primary text-white rounded px-3 py-2 mb-4">
      <h4 class="mb-0">Lista Cursos Cadastrados</h4>
    </div>
    <?php if (!empty($cursos)): ?>
        ...
        <table class="table table-bordered align-middle table-striped">
          ...
          <tbody>
            <?php foreach ($cursos as $row): ?>
              <tr>
                <td class="text-center"><?= htmlspecialchars($row['curso']) ?></td>
                <td class="text-center"><?= htmlspecialchars($row['sigla']) ?></td>
                <td class="text-center"><?= htmlspecialchars($row['curso_desc']) ?></td>

                <td class="text-center">
                  <a href="../updates/updates_cursos.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Editar</a>
                  <a href="../deletes/deletes_cursos.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Deseja excluir este registro?')">Excluir</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
