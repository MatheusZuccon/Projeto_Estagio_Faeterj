...
$uf = [];
try {
    $stmt = $conn->prepare("SELECT * FROM faeterj_apoio_uf");
    $stmt->execute();
    $uf = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "Erro ao buscar unidades: " . $e->getMessage();
}
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <div class="container mt-5 mb-5">
    ...
    <div class="bg-primary text-white rounded px-3 py-2 mb-4">
      <h4 class="mb-0">Lista de UFs Cadastrados</h4>
    </div>
        ...
        <table class="table table-bordered align-middle table-striped">
          ...
          <tbody>
            <?php foreach ($uf as $row): ?>
              <tr>
                <td class="text-center"><?= htmlspecialchars($row['Serv_DescUF']) ?></td>
                <td class="text-center">
                  <a href="../updates/updates_uf.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Editar</a>
                  <a href="../deletes/deletes_uf.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Deseja excluir este registro?')">Excluir</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
