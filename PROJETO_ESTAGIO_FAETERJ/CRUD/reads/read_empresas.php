...
try {
    $sql = "SELECT * FROM faeterj_apoio_empresa";
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <div class="container mt-5 mb-5">
    ...
    <div class="bg-primary text-white rounded px-3 py-2 mb-4">
      <h4 class="mb-0">Lista de Empresas Credenciadas</h4>
    </div>
          ...
          <tbody>
            <?php foreach ($resultados as $row): ?>
              <tr>
                ...
                <td><?= htmlspecialchars($row['telefone_fixo']) ?></td>
                <td><?= htmlspecialchars($row['cidade']) ?></td>
                <td class="text-center">
                  <a href="../updates/updates_empresas.php?empresa=<?= $row['empresa'] ?>" class="btn btn-sm btn-warning">Editar</a>
                  <a href="../deletes/deletes_empresas.php?empresa=<?= $row['empresa'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Deseja excluir este registro?')">Excluir</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
